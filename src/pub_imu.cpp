...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("[IMU] port open checkpoint");
...
//        for(int j=i;j<i+MAX_PACKET_LEN;j++)
//        {
//            printf(" %X ", buf[j]);
//        }
//        printf(" \n\n ");
        ...
        */
        ...
        if(buf[i+2] == kItemAccRaw)  /* Acc raw value */
        {
        ...
        if(buf[i+9] == kItemGyoRaw)  /* gyro raw value */
        {
...
//            imu_msg. = GyoRaw[0];
        ...
        if(buf[i+23] == kItemAtdE)  /* atd E */
        {
            ...
            double radian = (Eular[2] * 3.1415926) / 180;
            double angle = atan2(sin(radian), cos(radian));
            ROS_INFO("yaw: %f", angle);
